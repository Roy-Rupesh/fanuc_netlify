...
import { fetchAllowedMachines } from "../../../redux/utilityDemoSlice";
...
const CombinedCharts: React.FC = () => {
  ...
  useEffect(() => {
    if (demoId) {
      dispatch(fetchAllowedMachines(demoId) as any).then((action: any) => {
        if (fetchAllowedMachines.fulfilled.match(action)) {
          // Set the allowed machines state with the response data
          setAllowedMachines(action.payload.machineList);
        }
      });
    }
  }, [dispatch, demoId]);
  ...
  useEffect(() => {
    if (allowedMachines.length > 0 && !selectedMachine) {
      setSelectedMachine(allowedMachines[0].machineName);
    }
  }, [allowedMachines, selectedMachine]);
